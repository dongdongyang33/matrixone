...
func cnMessageHandle(receiver *messageReceiverOnServer) error {
	...
	case pipeline.PrepareDoneNotifyMessage: // notify the dispatch executor
		...
		fmt.Printf("[msghandler] success cnt = %d\n", successCnt)
		hasErr := false
		...
		timeoutCtx, cancel := context.WithTimeout(context.Background(), SendEndMessageTimeout)
		_ = cancel
		message := cnclient.AcquireMessage()
		{
			message.Id = receiver.messageId
			message.Cmd = pipeline.BatchMessage
			message.Sid = pipeline.MessageEnd
			message.Uuid = receiver.messageUuid[:]
		}
		if retErr != nil {
			message.Err = pipeline.EncodedMessageError(timeoutCtx, retErr)
		}
		receiver.clientSession.Write(timeoutCtx, message)
