...
func (c *Compile) AddParallelDispatchScope(idx int, sender *Scope, receivers []*Scope, n *plan.Node) {
	if sender.NodeInfo.Mcpu == 1 { // don't need to parallel
		sender.Instructions = append(sender.Instructions, vm.Instruction{
			Op:  vm.Dispatch,
			Arg: constructBroadcastDispatch(idx, receivers, sender.NodeInfo.Addr, n),
		})
	...
	for i := range concurrScopes {
		...
		dispatch := constructBroadcastDispatch(idx, receivers, sender.NodeInfo.Addr, n)
		concurrScopes[i].Instructions = append(concurrScopes[i].Instructions, vm.Instruction{
			Op:  vm.Dispatch,
			Arg: dispatch,
		})
